...
# --- UPDATED DATABASE CONNECTION WITH FALLBACK ---
# Define the SQLite fallback path first
...
if pg8000 and all([DB_HOST, DB_USER, DB_PASS, DB_NAME]):
    ...
    try:
        ...
        print(f"❌ Postgres connection failed (Timeout/Firewall): {e}")
    ...
    print("⚠️ Postgres config missing or pg8000 not installed — falling back to SQLite.")
    app.config['SQLALCHEMY_DATABASE_URI'] = sqlite_uri

if not use_postgres:
    print(f"🗃️ Using SQLite at: {sqlite_path}")
...
class UserDataService:
    ...
    def save_user_story(self, data, images):
        ...
        # ### --- SMOOTHNESS_UPDATE --- ###
        # Clear the cache whenever a new story is saved
        cache.delete('view/api/buyer-collection')
        print("✅ Cache cleared for /api/buyer-collection.")
        # ### --- END UPDATE --- ###
...
def register():
    ...
    try:
        ...
        return jsonify(success=False, error=f"Database error during registration: {e}"), 500
...
def login():
    try:
        ...
        return jsonify(success=False, error=f"Database error during login: {e}"), 500
...
# ### --- SMOOTHNESS_UPDATE --- ###
# Cache this entire function for 300 seconds (5 minutes)
                        ...
                        try:
                            ...
                            try:
                                ...
                                card_data = {
                                    ...
                                    'price': story_data.get('price', 0), # <-- Pass price to frontend
                                    'imageUrl': image_blob_name,
                                    'arPreviewUrl': ar_preview_blob_name,
...
if __name__ == '__main__':
    ...
    with app.app_context():
        try:
            ...
            # This will now use SQLite if the Postgres connection failed the test
            db.create_all()
            print("✅ Database tables ensured.")
        except Exception as e:
            # This catch is still important in case the fallback SQLite fails
