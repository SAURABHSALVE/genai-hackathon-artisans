...
#             # Use a model compatible with the GenerativeAI library
...
#             return self._fallback_story(data)
...
#         except Exception as e:
#             # Catch potential API errors (e.g., blocked content)
#             print(f"❌ Error generating story with Google AI: {str(e)}")
#             # Check if response object has safety feedback if error isn't JSON decode
#             if 'response' in locals() and hasattr(response, 'prompt_feedback'):
#                  print(f"Prompt Feedback: {response.prompt_feedback}")
#             if 'response' in locals() and hasattr(response, 'candidates') and response.candidates and hasattr(response.candidates[0], 'finish_reason'):
#                  print(f"Finish Reason: {response.candidates[0].finish_reason}")
#             if 'response' in locals() and hasattr(response, 'candidates') and response.candidates and hasattr(response.candidates[0], 'safety_ratings'):
#                  print(f"Safety Ratings: {response.candidates[0].safety_ratings}")

#             return self._fallback_story(data, error_message=str(e))
