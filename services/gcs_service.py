...
class GCSService:
    ...
    def delete_file(self, blob_name):
        ...
        try:
            ...
            if blob.exists():
                ...
                print(f"⚠️ File {blob_name} does not exist in GCS")
                return {'success': False, 'error': 'File not found'}
        except Exception as e:
            print(f"❌ GCS delete error: {e}")
            return {'success': False, 'error': str(e)}
    ...
    def file_exists(self, blob_name):
        ...
        try:
            ...
            if blob_name.startswith(f"https://storage.googleapis.com/{self.bucket_name}/"):
                blob_name = blob_name[len(f"https://storage.googleapis.com/{self.bucket_name}/"):]
            elif blob_name.startswith(f"gs://{self.bucket_name}/"):
                blob_name = blob_name[len(f"gs://{self.bucket_name}/"):]
            elif blob_name.startswith("http://localhost:3001/api/get-image/"):
                blob_name = blob_name[len("http://localhost:3001/api/get-image/"):]
            blob = self.bucket.blob(blob_name)
            exists = blob.exists()
            print(f"✅ Checked file existence for {blob_name}: {'exists' if exists else 'does not exist'}")
            return exists
        except Exception as e:
            print(f"❌ Error checking existence of {blob_name}: {e}")
            return False
